...
function FlightSearch() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.get(`${process.env.REACT_APP_URL}/flights/search`, { 
        params: searchParams
      });
  ...
  const submitBooking = async (bookingData) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_URL}/bookings/create`, payload, {
        headers: {
