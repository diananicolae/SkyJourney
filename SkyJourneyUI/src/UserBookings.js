...
function UserBookings() {
  ...
  const fetchBookings = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_URL}/bookings/user/${userId}`, {
        headers: {
  ...
  const handleCheckIn = async (bookingId) => {
    try {
      await axios.put(`${process.env.REACT_APP_URL}/bookings/${bookingId}/check-in`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleCancel = async (bookingId) => {
    try {
      await axios.put(`${process.env.REACT_APP_URL}/bookings/${bookingId}/cancel`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
